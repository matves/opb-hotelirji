...
def get_user(auto_login = True):
	...
	if uporabnisko_ime is not None:
		...
		c.close ()
		if r is not None:
			# uporabnik obstaja, vrnemo njegove podatke (uporabniško ime, ime, oid)
			return r
...
def main():
	...
	# Odvisno od uporabnika se kliče funkcija, ki vrne ustrezno tabelo in jo vstavi na mesto v mainu.
...
def nova_rezervacija():
	...
	rezervacija=bottle.request.forms.Rezervirajte
	...
	# On pritisne informativni izračun, če je soba frej, izpiše informativno ceno, če ne mu napiše, da ni frej.
		...
		cur.execute("""SELECT sid FROM soba LEFT JOIN termin ON soba.sid=termin.soba WHERE soba.tip=%s AND soba.kapaciteta=%s AND 
					(termin.zacetek IS NULL OR (SELECT (to_date(%s, 'YYYY-MM-DD') - to_date(to_char(termin.konec, 'YYYY-MM-DD'), 'YYYY-MM-DD'))>=0) OR 
					(SELECT (to_date(%s, 'YYYY-MM-DD') - to_date(to_char(termin.zacetek, 'YYYY-MM-DD'), 'YYYY-MM-DD'))<=0)) AND 
					sid NOT IN 
					...
					[str(soba_tip),kapaciteta,str(zacetek),str(konec.isoformat()),str(soba_tip),kapaciteta,str(zacetek.isoformat()),str(konec.isoformat())])
